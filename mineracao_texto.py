...
while Ctrl!='s':
    ...
    def AplicStemmer(texto):
        stemmer = nltk.stem.RSLPStemmer()
        frasesstemerless = []
        for (palavras, emocao) in texto:
            comstemming = [str(stemmer.stem(p)) for p in palavras.split() if p not in stopwordsnltk]
            frasesstemerless.append((comstemming, emocao))
        return frasesstemerless

    StremmingPhraseTrain = AplicStemmer(BaseTreinamento)
    StremmingPhraseTest = AplicStemmer(BaseTeste)
    ...
    WordsTrain = BuscaPalavras(StremmingPhraseTrain)
    WordsTest = BuscaPalavras(StremmingPhraseTest)

    def BuscaFrequencia(palavra):
        palavras = nltk.FreqDist(palavra)
        return palavras

    FreqTrain = BuscaFrequencia(WordsTrain)
    FreqTest = BuscaFrequencia(WordsTest)

    UniqueWordsTrain = FreqTrain.keys()
    UniqueWordsTest = FreqTest.keys()

    def ExtratorPalavras(documento):
        doc = set(documento)
        caract = {}
        for palavras in UniqueWordsTrain:
            caract['%s' % palavras] = (palavras in doc)
        return caract

    BaseCompletaTrain = nltk.classify.apply_features(ExtratorPalavras, StremmingPhraseTrain)
    BaseCompletaTest = nltk.classify.apply_features(ExtratorPalavras, StremmingPhraseTest)

    #Classifier : constroi a tabela de probabilidade da base treinamento.
    ...
    while ((QuestMatriz!= 's') and (QuestMatriz!= 'n')):
    ...
    def TratarFrase(texto):
        ...
        for palavra in texto.split():
            constem = [ p for p in palavra.split()]
            testestemming.append(str(stemmer.stem(constem[0])))
        return testestemming

    NovaFrase = input('Digite uma frase:')
    teststremming = TratarFrase(NovaFrase)
    novo = ExtratorPalavras(teststremming)
    #print(novo)

    print('A emoção extraida da frase é: {}\n'.format(Classifier.classify(novo)))
    ...
    while ((StatusClassific != 's') and (StatusClassific != 'n')):
        StatusClassific = input('Favor responder usando s ou n. \n A emoção está correta? s/n')

    if  StatusClassific=='n':
        ind=1
        for i in Labels:
            print('{} - {}\n'.format(ind,i))
            ind+=1
        ind=1
        EmotionClassific = int(input('Escolha a emoção que melhor defini a frase(1 a 6):\n'))
        while ((EmotionClassific < 1) or (EmotionClassific > 6)):
            EmotionClassific = int(input('Favor responder usando umas das opções acima(1 a 6):'))

        for i in Labels:
            if  EmotionClassific == ind:
                NovaEntrada = (NovaFrase,i)
                print(NovaEntrada)
                break
            ind+=1
